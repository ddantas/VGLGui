...
#
#
sub PrintExecFile { # ($basename, $comment, $semantics, $type, $variable, $default, $is_array, $is_shape, $size, $output, $cpp_read_path) {
  my $basename      = $_[0];
  my $comment       = $_[1];
  my $semantics     = $_[2];
  my $type          = $_[3];
  ...
  for (my $i = 0; $i <= $#type; $i++) {
      if ($i == 0 || $i == 1 || $i == 2 && $semantics[$i] eq "__write_only") {
        ...
        $has_window = 1;
        print PYTHON "          window = getImageInputByIdName(vGlyph.glyph_id, 'window', workspace)\n";
        push @params, "window";
    }
